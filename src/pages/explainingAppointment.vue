<template>
  <div class="wrap">
    ...
    <div class="title">
        <p class="title1">Explaining appointment</p>
        <p class="title2">讲解预约</p>
    </div>
    <div class="content">
        <!-- <p class="title">{{title}}</p> -->
        <p class="artical" v-html="content" ref="artical">
        </p>
    </div>
  </div>
</template>
<script>
import { explainInfo, wxInfo } from '../http/interface'
...
export default {
    ...
    methods: {
        ...
        async getVenueIntroduction () {
            ...
            try {
                vm.app.showLoading()
                const res = await explainInfo()
                vm.default_img = res.data.default_img
                vm.imgUrls = res.data.imgs
                vm.content = res.data.detail
